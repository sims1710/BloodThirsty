...
[CreateAssetMenu(fileName = "BloodStorageUpgrade", menuName = "ScriptableObjects/BloodStorageUpgrade", order = 1)]
public class BloodStorageUpgrade : ScriptableObject, IUpgradeable
{
    ...
    {
        if (PlayerPrefs.HasKey("BloodStorageLevel"))
        {
            currentLevel = PlayerPrefs.GetInt("BloodStorageLevel");
        }
        else
        {
            currentLevel = 0;
            PlayerPrefs.SetInt("BloodStorageLevel", currentLevel);
            PlayerPrefs.Save();
        }
        return currentLevel;
    }
